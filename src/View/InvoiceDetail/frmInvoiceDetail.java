 ...
 * @author Manh
 */
public class frmInvoiceDetail extends javax.swing.JPanel {
     ...
     */
    public frmInvoiceDetail() throws SQLException {
        initComponents();
        String[] colsName = {"Id", "Invoice Number", "Product Name", "Quantity", "UnitPrice", "Total Amount"}; // Đặt tiêu đề cột cho tableModel
    ...
    public void ShowData() throws SQLException {
        List<InvoiceDetail> lst = ivdt.getInvoiceDetailById(); // Lấy danh sách loại sản phẩm từ cơ sở dữ liệu
        try {
            // Xóa dữ liệu cũ trước khi thêm mới
            tableModel.setRowCount(0);
            for (InvoiceDetail item : lst) { // Duyệt qua từng dòng của lst
                Object[] rows = {item.getInvoiceDetailID(), item.getInvoice().getIvNumber(), item.getProduct().getPdName(), item.getQuantity(), item.getUnitPrice(), item.getTotalAmount()};
...
//                    ivdt.DeleteData(id);//goi ham xoa du lieu theo ma loai
